...
- Providers so precisam mapear o webhook bruto para o DTO e informar
  o channel; o author e resolvido pelo servico comum.
...
3. Chamar ``self.env["mail.gateway.whatsapp.common"]._process_normalized(gateway, dto, channel)``.
4. Manter a logica de resolucao de canal no provider (cada API tem sua forma).
